    ...
    : ISyncService, IStoppableService, ISessionStateAware, IMappingsSetupStateAware, IOfflineStateAware, IMappedFileSystemIdentityProvider, ISyncRootPathProvider
{
    ...
    public void Resume()
    {
        ApplyPause(paused: false, pauseUntil: null);
    }

    public void PauseUntil(DateTimeOffset? time)
    {
        ApplyPause(paused: true, pauseUntil: time);
    }
    ...
    private void ApplyPause(bool paused, DateTimeOffset? pauseUntil)
    {
        ...
        lock (_lock)
        {
            ...
            settings.PausedUntil = paused ? pauseUntil : null;
    ...
    private void OnStateChanged(SyncStatus status, bool failed)
    {
        ...
        var state = new SyncState(status, failed, pausedUntil);
