    ...
    : PageViewModel, ISessionStateAware, ISyncStateAware, ISyncActivityAware, ISyncStatisticsAware, IDisposable
{
        ...
        IClock clock)
    {
        ...
        PauseSyncTemporarilyCommand = new RelayCommand<object?>(PauseSyncTemporarily);
        PauseSyncUntilTomorrowMorningCommand = new RelayCommand(PauseSyncUntilTomorrowMorning);
        ResumeSyncCommand = new RelayCommand(ResumeSync);
    ...
    public ICommand PauseSyncTemporarilyCommand { get; }
    public ICommand PauseSyncUntilTomorrowMorningCommand { get; }
    public ICommand ResumeSyncCommand { get; }

    public SyncStatus SynchronizationStatus => _synchronizationState.Status;

    public string? FormattedSynchronizationPausedUntil
    {
        get
        {
            if (_synchronizationState.PausedUntil is not { } pausedUntil)
            {
                return null;
            }
    ...
    public bool Paused
    {
        get => _syncService.SyncIsPaused;
        set
        {
            if (_syncService.SyncIsPaused == value)
            {
                return;
            }
    ...
    void ISyncStateAware.OnSyncStateChanged(SyncState value)
    {
        _scheduler.Schedule(
            () =>
            {
                ...
                OnPropertyChanged(nameof(FormattedSynchronizationPausedUntil));
    ...
    private void PauseSyncTemporarily(object? parameter)
    {
        if (parameter is not string value || !int.TryParse(value, out int numberOfHours))
        {
            return;
        }
