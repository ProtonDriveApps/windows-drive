...
public class LoggingOuterHttpMessageHandler : DelegatingHandler
{
    ...
    internal static class Log
    {
        ...
        private static readonly Action<ILogger, int, HttpMethod, string?, int, double, int, Exception?> LogRequestPipelineEndWarning = LoggerMessage.Define<int, HttpMethod, string?, int, double, int>(
            LogLevel.Warning,
            EventIds.PipelineEnd,
            "End processing   - {RequestId:000000} {HttpMethod} {Uri}, attempts: {NumberOfAttempts}, after {ElapsedMilliseconds:0.00} ms - {StatusCode}");
        ...
        public static void RequestPipelineEnd(ILogger logger, HttpRequestMessage request, HttpResponseMessage response, TimeSpan duration)
        {
            ...
            {
                // Success on second or subsequent attempt is logged at the Information logging level
            ...
            {
                // Failure is logged at the Warning logging level
                LogRequestPipelineEndWarning(
                    logger,
                    requestId,
                    request.Method,
                    GetUriString(request.RequestUri),
