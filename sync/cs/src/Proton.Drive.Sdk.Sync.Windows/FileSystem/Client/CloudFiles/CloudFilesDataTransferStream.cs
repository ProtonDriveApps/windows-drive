...
internal sealed class CloudFilesDataTransferStream : Stream
{
    ...
    private unsafe void TransferChunk(ReadOnlySpan<byte> chunk)
    {
        fixed (byte* chunkPointer = &MemoryMarshal.GetReference(chunk))
        {
            ...
            {
                _logger.LogDebug(
                    "TRANSFER_DATA for file with Id={Id} (TransferKey={TransferKey}, RequestKey={RequestKey}, Offset={Offset}, Length={Length})",
                    _id,
                ...
                {
                    _logger.LogWarning(
                        "TRANSFER_DATA for file with Id={Id} (TransferKey={TransferKey}, RequestKey={RequestKey}, Offset={Offset}, Length={Length}) failed ({HResult})",
                        _id,
                        _operation.TransferKey.GetHashCode(),
                        _operation.RequestKey.GetHashCode(),
                        _transferPosition,
    ...
    private void ValidateChunk(int length)
    {
        ...
        {
            _logger.LogDebug(
                "ACK_DATA for file with Id={Id} (TransferKey={TransferKey}, RequestKey={RequestKey}, Offset={Offset}, Length={Length})",
                _id,
                _operation.TransferKey.GetHashCode(),
                _operation.RequestKey.GetHashCode(),
                _transferPosition,
            ...
            {
                _logger.LogWarning(
                    "ACK_DATA for file with Id={Id} (TransferKey={TransferKey}, RequestKey={RequestKey}, Offset={Offset}, Length={Length}) failed ({HResult})",
                    _id,
                    _operation.TransferKey.GetHashCode(),
                    _operation.RequestKey.GetHashCode(),
                    _transferPosition,
