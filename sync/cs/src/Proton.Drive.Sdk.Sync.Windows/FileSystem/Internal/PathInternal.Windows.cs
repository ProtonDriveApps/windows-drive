...
namespace Proton.Drive.Sdk.Sync.Windows.FileSystem.Internal;
...
internal static partial class PathInternal
{
    ...
    /// <summary>
    /// Adds the extended path prefix (\\?\) if not already a device path, IF the path is not relative,
    /// AND the path is more than 247 characters. (> 248 + null). This will also insert the extended
    /// prefix if the path ends with a period or a space. Trailing periods and spaces are normally eaten
    /// away from paths during normalization, but if we see such a path at this point it should be
    /// normalized and has retained the final characters. (Typically from one of the *Info classes)
    /// </summary>
    [return: NotNullIfNotNull("path")]
    internal static string? EnsureExtendedDirectoryPathPrefixIfNeeded(string? path)
    {
        if (path != null && (path.Length >= MaxShortDirectoryPath || EndsWithPeriodOrSpace(path)))
        {
            return EnsureExtendedPrefix(path);
        }
        else
