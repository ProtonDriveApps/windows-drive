...
public sealed class SkiaThumbnailGenerator : IThumbnailGenerator
{
    ...
    public SkiaThumbnailGenerator(ILogger<IThumbnailGenerator> logger)
    {
        _logger = logger;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new ThumbnailGenerationException(ThumbnailGenerationErrorCode.FormatNotSupported, "Failed to decode image");
        }
        ...
        if (isHdPreview)
        {
            var imageNumberOfPixelsOnLargestSide = Math.Max(skiaImage.Height, skiaImage.Width);
        ...
        using var originalBitmap = SKBitmap.FromImage(skiaImage);
