    ...
    where TAltId : IEquatable<TAltId>
{
        ...
        AdapterTreeNodeModel<TId, TAltId>? previousNodeModel)
    {
        ...
        RecordDetectionIfExcelTemporaryFileCandidate(operation);
    }

    private void RecordDetectionIfExcelTemporaryFileCandidate(Operation<AdapterTreeNodeModel<TId, TAltId>> operation)
    {
        if (operation.Type is OperationType.Create
            && operation.Model.Type is NodeType.File
            && MicrosoftExcelTemporaryFile.IsCandidate(operation.Model.Name))
        {
            _excelTemporaryFileDetectionCounter.Increment();
        }
    }
